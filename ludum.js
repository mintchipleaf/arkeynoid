...
function checkKB(scene){
	...
	for (var i = 0; i < keys1.length; i++) {
			...
			if(game.keyboard.consumePressed("space") || game.keyboard.consumePressed("enter")){
				key = true;
			}
	}
	return key;
...
function addBall(){
	...
	var ball = new Splat.AnimatedEntity(canvas.width / 2 - (ballImg.width /2), canvas.height - 100, ballImg.height, ballImg.width, ballImg, 0, 0);
	if(balls.length == 0){
		speed = 0.5;
	}
	if(Math.random() > 0.5){
...
game.scenes.add("title", new Splat.Scene(canvas, function() {
	...
	paddle = new Splat.AnimatedEntity(canvas.width / 2 - (paddleImg.width /2) , canvas.height - 50, paddleImg.width, paddleImg.height, paddleImg, 0, 0);
...
if(gameOver || gameWon){
	addBall(); //Hell yeah
	if(game.keyboard.isPressed("space") || game.keyboard.isPressed("enter")){
		waitingToStart = true;
...
if(game.keyboard.isPressed("shift")){
	...
	if(waitingToStart){
		this.camera.drawAbsolute(context, function(){
				...
				context.font = "155px Arkanoid";
				context.fillText("ARKEYNOID", 0, canvas.height / 2 + 60);
				context.font = "50px arial";
				context.fillText("Press any key to begin", 270, canvas.height / 2 + 150);
			});
	}
	...
	if(gameOver && !gameWon){
		//this.camera.drawAbsolute(context, function() {
			context.fillStyle = "#ffffff";
			context.font = "155px Arkanoid";
			context.fillText("You Lost", 20, canvas.height / 2 + 60);
			context.font = "50px arial";
			context.fillText("Press any key to try again", 210, canvas.height / 2 + 110);
	...
	if(gameWon){
		context.fillStyle = "#ffffff";
		context.font = "155px Arkanoid";
		context.fillText("You Won", 70, canvas.height / 2 + 60);
		context.font = "50px arial"; 
		context.fillText("Press space to restart", 260, canvas.height / 2 + 150);
	}
